				...
				if (
					...
					if (!imgElem) {
						...
						if (
							...
							var target = svgNode.getAttribute("target");
							if (target == "_blank") {
								// target="_blank" の場合は読み込みをスキップし、空のSVGを登録する 2026/01/23
								this.#svgImages[imageId] = new DOMParser().parseFromString(
									'<svg xmlns="http://www.w3.org/2000/svg"></svg>',
									"application/xml"
								);
								this.#svgImagesProps[imageId] = new SvgImageProps();
								this.#svgImagesProps[imageId].Path = UtilFuncs.getImageURL(
									ip.href,
									docDir
								);
